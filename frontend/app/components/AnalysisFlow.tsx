...
interface AnalysisFlowProps {
  initialUpload?: UploadResponse;   // if provided, skip Step 1 and start at Step 2
  intervalHint?: string;            // shown as a banner on the Interval step
  calibrationHint?: string;         // shown as a banner on the Calibrate step
  onExit?: () => void;              // e.g. "back to library" — adds an extra exit button
  exitLabel?: string;
}
...
export default function AnalysisFlow({
  ...
  const [step, setStep] = useState(initialUpload ? 2 : 1);
  const [uploadData, setUploadData]   = useState<UploadResponse | null>(initialUpload ?? null);
  const [startFrame, setStartFrame]   = useState(0);
  const [endFrame, setEndFrame]       = useState(initialUpload ? initialUpload.total_frames - 1 : 0);
  ...
  function handleBack() {
    if (initialUpload && step === 2) return; // no Upload step to go back to
    setStep(s => s - 1);
  }

  function resetFlow() {
    if (initialUpload) {
      // Guided mode: keep the same pre-loaded video, just rewind to Interval
      setStep(2);
      setResult(null); setCalibration(null); setAnalysis(null);
      setStartFrame(0); setEndFrame(initialUpload.total_frames - 1);
    } else {
      setStep(1); setResult(null); setUploadData(null);
      setCalibration(null); setAnalysis(null); setMethod("yolo");
    }
  }
  ...
  function copySampleConfig() {
    ...
    const snippet = {
      ...
      useAirResistance,
      intervalHintText: "REPLACE ME",
      calibrationHintText: "REPLACE ME",
    };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f9fa", color: "#111827", fontFamily: "system-ui, sans-serif" }}>
      <Navbar currentStep={step} onLogoClick={onExit ?? resetFlow} />
      ...
      <main style={{ maxWidth: step === 5 ? "1440px" : "900px", margin: "0 auto", padding: "40px 2rem", transition: "max-width 0.25s ease" }}>
        ...
        <div style={{
          ...
          {step === 2 && uploadData && (
            <>
              {intervalHint && <HintBanner text={intervalHint} />}
              <div style={{
          ...
          {step === 3 && uploadData && (
            <>
              {calibrationHint && <HintBanner text={calibrationHint} />}
              <CalibrationPicker
                videoId={uploadData.video_id}
                startFrame={startFrame}
                endFrame={endFrame}
        ...
        {/* Back */}
        {step > 1 && step < 5 && !(initialUpload && step === 2) && (
        ...
        {step === 5 && (
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            ...
            >
              {initialUpload ? "↺ Try again" : "↺ Start new analysis"}
            </button>
            {onExit && (
              <button
                onClick={onExit}
        ...
        {/* Dev mock button — only in local dev, only for the normal (non-guided) flow */}
        {process.env.NODE_ENV === "development" && !initialUpload && (
...
function HintBanner({ text }: { text: string }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: "13px", color: "#581c87", lineHeight: 1.5 }}>{text}</span>
    </div>
  );
}
